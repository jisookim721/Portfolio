...
for item in current_portfolio:
    ...
    # 수익률 계산 및 소수점 둘째 자리 라운딩
    raw_return_rate = (profit_loss / invested) * 100 if invested > 0 else 0
    return_rate = round(raw_return_rate, 2)
    ...
    portfolio_data.append({
        ...
        "평가금액": current_value,
        "수익금": profit_loss,
        "수익률": return_rate,  # 소수점 둘째 자리까지 라운딩된 값 전달
    })
...
total_return_rate = round(
    (total_profit / total_invested) * 100 if total_invested > 0 else 0, 2
)
...
if not df.empty:
    ...
    # 텍스트/호버 템플릿에 %{customdata[0]:+.2f}%를 사용하여 항상 소수점 둘째 자리까지 고정 표시
    fig.update_traces(
        texttemplate=(
