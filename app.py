...
# --- 4. 사이드바 메뉴 (URL 파라미터 기반 상태 동기화) ---
account_options = ("ISA 계좌", "연금 계좌", "위탁종합 계좌")

# URL 쿼리 파라미터에서 현재 계좌 정보 읽어오기
query_account = st.query_params.get("account", "ISA 계좌")

# URL 파라미터 값이 유효하지 않은 경우 기본값 설정
if query_account not in account_options:
    query_account = "ISA 계좌"

default_index = account_options.index(query_account)


# 라디오 버튼 변경 시 URL 파라미터를 업데이트하는 콜백 함수
def update_account():
    st.query_params["account"] = st.session_state["account_selector"]
...
# 페이지 접근 시 바로 URL 파라미터 보장
if "account" not in st.query_params:
...
total_return_rate = (
    (total_profit / total_invested) * 100 if total_invested > 0 else 0.0
)
...
# --- 8. Finviz 스타일 트리맵 ---
st.markdown(f"### 🗺 {selected_account} 포트폴리오 맵")
...
if not df.empty:
    ...
    # 트리맵 텍스트에 들어갈 소수점 2자리 포맷팅 텍스트 생성
    ...
    fig.update_layout(
        ...
        coloraxis_showscale=False,  # 우측 수익률 색상 범례(Colorbar) 제거
    )
